...
# python chair.py --image_id_key image
...
def load_generated_captions(cap_file, image_id_key:str, caption_key:str):
    ...
    # list of int
    # imids = [int(obj[image_id_key].split(".jpg")[0][-6:]) for obj in caps]  # vcd和agla的测试
...
def distinct_n_for_sentence(text, n):
...
def evaluate_metrics(file_path):
    ...
    d1_scores = [distinct_n_for_sentence(c, 1) for c in captions]
    d2_scores = [distinct_n_for_sentence(c, 2) for c in captions]
    ttr_scores = [lexical_richness_ttr(c) for c in captions]

    avg_d1 = sum(d1_scores) / len(d1_scores) if d1_scores else 0.0
    avg_d2 = sum(d2_scores) / len(d2_scores) if d2_scores else 0.0
    avg_ttr = sum(ttr_scores) / len(ttr_scores) if ttr_scores else 0.0

    return avg_d1, avg_d2, avg_ttr
...
if __name__ == '__main__':
    ...
    parser.add_argument("--coco_path", type=str, default='/home/amax/check/zla/database/COCO2014/annotations_trainval2014/annotations',
    ...
    fieldnames = ['CHAIRs', 'CHAIRi', 'Recall', 'Precision', 'F1', 'Len', ]

    # files=get_files_by_extension(directory, '.jsonl')
    # files=[
    #     "LLaVA/result/chair/coco/Greedy_7b/Greedy_7b.jsonl",
    #     "LLaVA/result/chair/coco/PAI_0.5/PAI_0.5.jsonl",
    #     "LLaVA/result/chair/coco/HGAI_0.5/HGAI_0.5.jsonl",
    #     "LLaVA/result/chair/coco/IAT_0.8/IAT_0.8.jsonl",
    #     "LLaVA/result/chair/coco/AdaIAT_6_0.5/AdaIAT_6_0.5.jsonl",
    # ]
    files=[
        "LLaVA/result/chair/Greedy_7b/Greedy_7b.jsonl",
        "LLaVA/result/chair/PAI_0.5/PAI_0.5.jsonl",
        "LLaVA/result/chair/HGAI_0.5/HGAI_0.5.jsonl",
        "LLaVA/result/chair/IAT_0.8/IAT_0.8.jsonl",
        "LLaVA/result/chair/AdaIAT_6_0.5/AdaIAT_6_0.5.jsonl",
    ]



    with open('chair.csv', 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()  # 写入表头    

        for cap_file in files:
            print(cap_file)
            name=get_filename_without_ext(cap_file)
            args.save_path=f"LLaVA/result/chair_res/res_{name}.json"

            cap_dict = evaluator.compute_chair(cap_file, args.image_id_key, args.caption_key) 

            print_metrics(cap_dict)
            for key in cap_dict['overall_metrics']:
                cap_dict['overall_metrics'][key]*=100
                cap_dict['overall_metrics'][key]=round(cap_dict['overall_metrics'][key],2)

            writer.writerow(cap_dict['overall_metrics'])
    ...
    # 输出 CSV 文件路径
    output_csv = "distinct.csv"

    with open(output_csv, mode='w', newline='', encoding='utf-8') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["Distinct-1", "Distinct-2"])
        for file_path in files:
            result = evaluate_metrics(file_path)
            if result:
                d1, d2, ttr = result
                writer.writerow([round(d1, 4), round(d2, 4)])
            else:
                print(f"[跳过] 文件不存在: {file_path}")

    with open('chair.csv', 'r') as f1, open('distinct.csv', 'r') as f2, open('metrics.csv', 'w', newline='') as out:
        ...
        writer.writerow(header1 + header2)  # 拼接表头
